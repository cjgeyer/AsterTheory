...
The only analyses known to us using R package \code{aster2} are
...
It will not be important in what follows, but a special subvector
...
message).
R function \code{tsort} in R package \code{pooh}
...
$q(G) \in H$.  These are supplied to R function \code{tsort} as arguments
\code{from} and \code{to}, which are vectors whose components are elements
of $\mathcal{G}$ (coded somehow), that is, when \code{from[k]}
is $G$, then \code{to[k]} is the $H$ that contains $q(G)$.
And every $G$ such that $q(G) \in J$ occurs in \code{from}.

Vectors \code{from} and \code{to} could have length zero.  This would happen
if $q(G) \notin J$ for all $G \in \mathcal{G}$.  This is only the most extreme
case where \code{from} does not contain all elements of $\mathcal{G}$.
When this happens, and we want the result to be a total order on $\mathcal{G}$,
we can provide $\mathcal{G}$ (in any order) as the \code{domain} argument of
R function \code{tsort}.

When invoked with these arguments, R function \code{tsort} returns
...
\item The elements of $\mathcal{G}$ are the maximal connected components
    ...
    R function \code{weak} in R package \code{pooh} \citep{pooh-package}
...
R package \code{aster2} does the Right Thing: uses ``initial'' node.
...
If $G$ is a dependence group whose
...
We want to use this notation with the index-to-index predecessor function
...
Aster graphs for ``individuals'' can get a lot bigger than \eqref{gr:aster1}.
...
Since cumulant functions are convex sets (Lemma~\ref{lem:convex} below),
...
It is helpful to have a term saying what model a canonical affine submodel
...
The algorithms used by R function \code{aster} in R package \code{aster}
have this property.
The algorithm used by R function \code{transformUnconditional} in
R package \code{aster2}
has this property.
...
collinearity, automatically.  R functions \code{lm} and \code{glm} in core R
...
The inverse mapping to the the mapping $\beta \mapsto \tau$ is calculated
