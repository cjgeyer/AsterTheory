...
They obviously imply, if $\hat{\theta}$ is a maximum likelihood estimate,
...
between these points.  Thus the maximum likelihood estimate for a full
...
The vector $\delta$ is a direction of constancy if an only if the random
...
$$
Then clearly, $\delta$ is a direction of recession if and only if
$$
   \var_\theta(\inner{Y, \delta}) = \delta^T I(\theta) \delta
$$
is equal to zero.  And by the spectral theorem
...
The inverse mappings to these mappings do not have closed form expression
...
\item the arrows $\xi \mapsto \theta$ and $\mu \mapsto \varphi$, which
    are unlabled fail to be maps when the canonical parameterizations
...
The next few sections introduce these more parsimonius models and
...
With unconditional canonical afffine submodels (preceding section) we get
...
preceding section, and the three unlabled arrows are parameter transformations
...
The theory of conditional aster models is impovrished compared to that
...
A point is a global maximizer of $l$ or $\tilde{l}$ if an only if the first
derivative is equal to zero.  Hence $\theta$ is a maximum likelihood estimate
...
Here we do not assume \eqref{eq:observed-equals-expected} has a solution.
