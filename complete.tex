...
The following theorem encapsuates the calculations of Slide~35~ff.\ of the
...
in R package \code{aster} that have only univariate dependence groups
...
in R package \code{aster2} that have not only univariate dependence groups
but also multinomial and normal-location-scale dependence groups.
Another feature of currently implemented aster models (both \code{aster}
and \code{aster2}) is that they do not allow zero values to be specified
for initial nodes.
% Immediately preceding sentence is TRUE for R package aster2 and FALSE
% for R package aster, as of version 1.0.3.  Probabily should add checks
% to R package aster to make this TRUE for it too.
\begin{proof}
The set of all directions of recession is a closed convex cone
